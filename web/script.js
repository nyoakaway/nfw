		...
		messageListener.addHandler("RegisterInterface", (data) => {
			...
			try {
                ...
                this.#loadingModules[moduleName] = new Promise((resolve, reject) => {
                    nui.load(`../modules/${moduleName}/web/index.html`, (data) => {
                        resolve(true)
    ...
    async #EnsureNuiLoad(moduleName) {
        ...
        if (nui.html() == "") {
            return await new Promise((resolve, reject) => {
                nui.load(`../modules/${moduleName}/web/index.html`, () => {
                    resolve(true);
                });
            });
        }
...
class Module {
    ...
    constructor(moduleName) {
        ...
        function setLang(lang) {
            ...
            fetch('https://cfx-nui-nfw/modules/' + self.moduleName + '/lang/' + lang + '.json').then(res => res.text().then(async text => {
...
async function FetchData(url, data, cb, path) {
	const res = await fetch(`https://nfw/${path ? path : nuiModuleHandler.getActiveModule() + '/' + url}`, {
		method: "POST",
		headers: {
